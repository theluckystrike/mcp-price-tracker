...
test.skip("pro license unlocks unlimited watches, refresh all and alerts_pending", async (t) => {
...
test.skip("pro history is not truncated at 10 observations", async (t) => {
  const key = execFileSync(process.execPath, [signer, "price-tracker"], { encoding: "utf8" }).trim();
  const c = client({ MCP_LICENSE_KEY: key });
  t.after(() => c.stop());
  await init(c);
